...
%% ------(Original) Frequency Domain------------------------------- 
    ...
    subplot(4,5,segment+10);
    hold on
    plot(0:(Fs/NFFT):(Fs/2-Fs/NFFT),P1(1:NFFT/2))
    xlabel('Frequency (Hz)'); 
    ylabel('voltage (mV)');
...
%% ------(With Noise) Frequency Domain------------------------------- 
    ...
    P1(2:end-1) = 2*P1(2:end-1);
    %subplot(4,1,3);
    plot(0:(Fs/NFFT):(Fs/2-Fs/NFFT),P1(1:NFFT/2),'R')
...
%% ------------optimize the threshold--------------------- 
 for test_threshold =1:100
    threshold=test_threshold/100;
    transform_array = dct(eye(N));
    if round(threshold*N)>0 && round(threshold*rem(Size(2),N))>0
...
%% -------------(Denoise) Frequency Domain---------------------
    ...
    P1 = P2(1:NFFT/2+1);
    P1(2:end-1) = 2*P1(2:end-1);
    plot(0:(Fs/NFFT):(Fs/2-Fs/NFFT),P1(1:NFFT/2),'G');
    txt=sprintf('FFT threshold= %f',MAX);
    title(txt);
    hold off
    
%% ----------------IFFT----------------------------
subplot(4,5,segment+15);
plot(t(1:Size(2)),Fix);
sprintf('Denoise PSNR for S2: %f', psnr(s,Fix,255));
xlabel('time (s)'); 
ylabel('voltage (mV)');
txt=sprintf('Dnoise Signal PSNR=%f', psnr(s,Fix,255));
title(txt);
%dim = [.8 .01 .3 .3];
